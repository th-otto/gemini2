...
/* internal texts
 */
#define NlsLocalSection "M.getopt"
enum NlsLocalText{
GO_AMBIG,	/*%s: Option '%s' ist mehrdeutig\n*/
GO_NOARG,	/*%s: Option '%c%s' darf kein Argument haben\n*/
GO_REQARG,	/*%s: Option '%s' ben”tigt ein Argument\n*/
GO_REQARG2,	/*%s: Option '-%c' ben”tigt ein Argument\n*/
GO_ILLOPT,	/*%s: Unbekannte Option '%c%s'\n*/
GO_ILLOPT2,	/*%s: Unbekannte Option, code=0%o\n*/
GO_ILLOPT3,	/*%s: Unbekannte Option '-%c'\n*/
...
typedef enum { REQUIRE_ORDER, PERMUTE, RETURN_IN_ORDER } Ordering;
...
static void exchange (MGLOBAL *M, GETOPTINFO *G, char **argv)
{
	size_t nonopts_size = 
		(G->last_nonopt - G->first_nonopt) * sizeof (char *);
	char **temp = (char **) mmalloc (&M->alloc, nonopts_size);
...
int getopt (MGLOBAL *M, GETOPTINFO *G, int argc, char **argv, 
	const char *optstring)
{
	...
	if (G->optind == 0)
	{
		...
		if (optstring[0] == '-')
			G->ordering = RETURN_IN_ORDER;
		else if (GetEnv (M, "_POSIX_OPTION_ORDER"))
			G->ordering = REQUIRE_ORDER;
		else
			G->ordering = PERMUTE;
	}
	...
	if (G->_getopt_long_options != 0  && 
		((argv[G->optind][0] == '-' && argv[G->optind][1] == '-')
		|| (G->_getopt_long_only && argv[G->optind][0] == '-')))
	{
		...
		/* Test all options for either exact match or abbreviated 
		 * matches.
		 */
		for (p = G->_getopt_long_options, G->option_index = 0; 
			p->name; p++, G->option_index++)
		...
		if (ambig && !exact)
		{
			eprintf (M, NlsStr(GO_AMBIG), argv[0], argv[G->optind]);
		...
		if (pfound != 0)
		{
			...
			if (*s)
			{
				...
				{
					eprintf (M, NlsStr(GO_NOARG), argv[0], 
						argv[G->optind - 1][0], pfound->name);
					G->nextchar += strlen (G->nextchar);			   
					return '?';
				}
			}
		...
		/* Can't find it as a long option.  If this is 
		 * getopt_long_only, and the option starts with '-' and is 
		 * a valid short option, then interpret it as a short option. 
	...
	
	{
		...
		if (temp == 0 || c == ':')
		{
			...
			{
				if (c < 040 || c >= 0177)
					eprintf (M, NlsStr(GO_ILLOPT2), argv[0], c);
				else
					eprintf (M, NlsStr(GO_ILLOPT3), argv[0], c);
			}
		...
		if (temp[1] == ':')
		{
			...
			else
			{
				...
				{
					if (G->opterr != 0)
						eprintf (M, NlsStr(GO_REQARG2), argv[0], c);
					c = '?';
				}
				else
				{
...
void optinit (GETOPTINFO *G)
{
	...
	G->opterr = 1;
	G->optind = G->_getopt_long_only = 0;
