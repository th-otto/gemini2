...
int
conv_date (MGLOBAL *M, const char *format, char *to)
{
...
size_t print_date (MGLOBAL *M, const char *format)
{
	...
	tmpstr = mmalloc (&M->alloc, format ? strlen (format) * 10 : 1000);	/* enough? */
	if (!tmpstr)
	{
		mprintf (M, StrError (ENSMEM));
		return 0;
	}
	
	conv_date (M, format, tmpstr);	
	len = mprintf (M, "%s", tmpstr);
