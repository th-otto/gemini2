/*
 *	@(#)Mupfel/cat.c
 *	@(#)Stefan Eissing & Julian F. Reschke, 16. September 1991
 */
...
/* internal texts
 */
#define NlsLocalSection "M.cat"
enum NlsLocalText{
CA_CANTOPEN,	/*cat: kann `%s' nicht ”ffnen\n*/
CA_CANTREAD,	/*cat: kann nicht aus %s lesen\n*/
CA_CANTWRT,		/*cat: kann nicht ausgeben\n*/
CA_DIRECT,		/*[Ende der Eingabe: UNDO oder CTRL-Z]\n*/
OPT_cat,		/*[-eilmnrstv] dateien*/
HLP_cat,		/*dateien auf stdout anzeigen*/
};
...
static long read_in (CATINFO *C, const char *file_name, int file_handle)
{
	...
	{
		eprintf (C->M, NlsStr(CA_CANTREAD), file_name);
		return 0;
	}
	else
	{
...
static int write_out (CATINFO *C, long amount)
{
	...
	if (C->tty_out)
	{
		...
		for (i = 0L; i < amount; i++)
		{
			canout (C->M, C->output_buffer[i]);
			if (intr (C->M)) return FALSE;
		}
...
static int cat_file (CATINFO *C, const char *file_name)
{
	...
	else if (!*file_name)
	{
		...
		file_handle = (int)Fopen (file_name, 0);
		
	if (file_handle < MINHND)
	{
		if (!C->silent)
		{
			eprintf (C->M, NlsStr(CA_CANTOPEN), file_name);
			return 2;	/* fatal */
		}
		else
			return 1;
...
int m_cat (MGLOBAL *M, int argc, char **argv)
{
	GETOPTINFO G;
	CATINFO C;
	...
	if (!argc)
		return PrintHelp (M, argv[0], NlsStr(OPT_cat), 
			NlsStr(HLP_cat));
	...
	while ((c = getopt_long (M, &G, argc, argv, "eilmnrstv", 
		long_options, &opt_index)) != EOF)
	{
		...
		switch (c)
		{
			...
			case 'v':
				...
				break;
			default:
				return PrintUsage (M, argv[0], NlsStr(OPT_cat));
		}
	}
	...
	if (argc == G.optind)
	{
		...
		if (C.tty_in)
			eprintf (M, NlsStr(CA_DIRECT));
			
		ret = cat_file (&C, NULL);		/* cat stdin */
	}
	else
	{
